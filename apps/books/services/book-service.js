...
function searchGoogleBooks(keyWord) {
    var books = storageService.loadFromStorage('searchBooks');
    if (!books || !books.length) {
        const prm = axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=effective%20${keyWord}`)
            .then(res => {
                storageService.saveToStorage('searchBooks', res.data.items)
